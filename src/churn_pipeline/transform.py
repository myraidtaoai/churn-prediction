...
if (
    silver.filter(F.col("customer_id").isNull() | (F.length(F.col("customer_id")) == 0))
    .limit(1)
    .count()
):
...
if (
    silver.filter(F.col("total_charges").isNull() & (F.col("tenure") != 0))
    .limit(1)
    .count()
):
...
).saveAsTable(table(args.catalog, args.schema, "telco_silver"))

summary = silver.groupBy("contract", "internet_service", "payment_method").agg(
